 ...
 */
export class CookieAuthorizationProvider extends AuthorizationProvider {
  /**
...
export class CookieAuthorizationProviderImpl extends CookieAuthorizationProvider {
  ...
  static fromJson(json) {
    const cookies = json.cookies.map(element => CookieContainer.fromJson(element));
    return new CookieAuthorizationProviderImpl(cookies);
  }
}
 ...
 * authorization such as FIDO UAF registration or de-registration.
 */
export class JwtAuthorizationProvider extends AuthorizationProvider {
  /**
   * The JWT.
   */
