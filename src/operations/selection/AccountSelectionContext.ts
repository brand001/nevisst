 ...
 */
export abstract class AccountSelectionContext {
	/**
...
export class AccountSelectionContextImpl extends AccountSelectionContext {
	...
	static fromJson(json: any): AccountSelectionContextImpl {
		const accounts = json.accounts.map((element: any) => Account.fromJson(element));
		const authenticators: Authenticator[] = json.authenticators.map((element: any) =>
			Authenticator.fromJson(element)
		);

		return new AccountSelectionContextImpl(
			json.operationId,
			accounts,
			authenticators,
			json.transactionConfirmationData
		);
	}

	async isPolicyCompliant(aaid: string, username: string): Promise<boolean> {
